...
const updateProductStatus = async () => {
  try {
    ...
    for (const product of products) {
      const createdAtPlusSevenDays = new Date(product.updatedAt);
      createdAtPlusSevenDays.setDate(createdAtPlusSevenDays.getDate() + 7);

      if (createdAtPlusSevenDays < currentDate) {
        // Nếu thời gian tạo cộng thêm 7 ngày nhỏ hơn thời gian hiện tại
        // Thì cập nhật trạng thái của sản phẩm
        await Product.updateMany(
          { _id: product._id, product_is_new: true },
          { $set: { product_is_new: false } }
        );
      }
    }
  } catch (error) {
