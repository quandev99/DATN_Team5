...
export const getProductFeature = async (req, res) => {
  ...
  try {
    const searchQuery = {
      ...
      variant_products: { $exists: true, $ne: [] },
    };
    ...
    if (minPrice || maxPrice || _color_id || _size_id) {
      ...
      const filteredProducts = products?.docs?.filter(product => {
        const hasMatchingVariant = product?.variant_products?.some(variant => {
    ...
    products.docs.sort((a, b) => b.average_score - a.average_score);

    products.docs.sort((a, b) => b.sold_quantity - a.sold_quantity);
