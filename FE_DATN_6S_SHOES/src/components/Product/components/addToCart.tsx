...
import { useGetProductByIdQuery, useGetVariantProductIDQuery } from "../../../api/product";
...
const AddToCart = ({ product }: any) => {
    ...
    const { data: variantGetOne } = useGetVariantProductIDQuery<any>(product?._id);
    const { data: productDetail } = useGetProductByIdQuery<IProduct>(selectedProductId as any);
    ...
    return (
        <div>
            ...
            >
                <div className='flex gap-3'>
                    <div className='max-h-[80px] max-w-[70px] border  min-h-[80px] min-w-[70px]'>
                        <img src={productDetail?.product?.product_image?.url} className='w-full h-full' alt="image" />
                    </div>
                    <div>
                        <div className='flex gap-2'>
                            <h1>Tên sản phẩm:</h1><p className='text-black font-medium'>{productDetail?.product?.product_name}</p>
                        </div>
                        <div>
                            <span>Số lượng: </span>
                                    ...
                                    {uniqueColorNames?.map((colorName: any, index) => (
                                        <li
                                            ...
                                            className={clsx(
                                                ...
                                                selectedColor === colorName && "border-blue-500 border-2 text-blue-600 font-medium"
                                            )}
                                    ...
                                    {uniqueSizeNames?.map((size, index) => (
                                        <li
                                            ...
                                            className={clsx(
                                                ...
                                                selectedSize === size && "border-blue-500 border-2 text-blue-600 font-medium",
            ...
            Mua hàng
                    ...
                    <button
                        disabled={validateValue() as any || disabled || false}
