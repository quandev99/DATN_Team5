...
import BuySpeed from './components/BuySpeed';
import { useMemo } from 'react';
import Swal from 'sweetalert2';
import { useAddToCartMutation } from '../../api/cart';

const ProductFeature = ({ product, isLoading }: any) => {
  ...
  return (
    <div
      ...
      <Link to="#" className="block  overflow-hidden group rounded-md">
                    ...
                    <Link to="#" className="block text-[20px] overflow-hidden group rounded-md" onClick={() => onHandleSubmit(product?._id)}>
                      <i
                        className={` ${!pro ? 'fa-regular' : 'fa-solid'
                          } transition-all duration-300 fa-heart fa-shake ml-2 hover:font-bold focus:font-bold text-red-600`}>
                      </i>
                    </Link>
                  </div>
                </p>
        ...
        <div className=" bg-gray-100 px-3 min-h-[70px] mb-10">
          <h3>
            <Link
